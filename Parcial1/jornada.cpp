    ...
    for(int i = 0; i < cantCursos; i++){
        ...
        for(int j = 0; j < HTI; j++){        //registra la hora y el dia de cada clase
            while(true){
                ...
                dia = dis1(gen);
                uniform_int_distribution<> dis2(6, 21);
                hora = dis2(gen);
                if(comparar(horario[dia][hora],libre)){
                    cout<<"hora"<<j+1<<endl<<"El "<<dias[dia]<<"a las "<<hora<<"esta disponible"<<endl<<"1. Usar espacio"<<endl<<"2. Sugerir otro espacio"<<endl<<"3. Asignar espacio manualmente"<<endl;
                    while(hora < 1 || hora > 3){
                        cin>>eleccion;
                    }
                    if(eleccion==1){
